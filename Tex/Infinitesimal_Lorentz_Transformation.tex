...
\noindent as $v^i = \gamma(u)(\vec{u},1)$. Here the usual convention that Greek indices denote the sum over the spacial indices only has been used, thus $\alpha = 1,2,3$. Now Eqn.(\ref{infinitesimal_v_interms_gamma}) can be used to rewrite the $\tensor{L}{^{i}_{j}}$ coefficients to get
