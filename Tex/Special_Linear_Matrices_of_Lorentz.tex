...
\noindent where $\vec{\sigma}$ are the Pauli matrices. The matrices $i\vec{\sigma}$ form a basis for the Lie algebra of the special unitary group $SU(2)$. This group will be dicussed briefly in Section (\ref{Fractional_Section_Extension_to_Minkowskian}). 
...
\noindent Hence the transformation $\vec{x} \rightarrow \vec{x}'$ implicit in Eqn.(\ref{SL_trans}) is a Lorentz transformation. This equation describes the most general POLT.
...
It is clear that there will always be two matrices, $\pm U$ corresponding to every Lorentz transformation, since if $U$ satisfies $A(\vec{x}') = U A(\vec{x}) U^{\dagger}$ then so does $-U$. Hence there is a $2$ to $1$ correspondence between the elements of $SL(2,\mathbb{C})$ and the POLTs, with $A$ and $A'$ corresponding to different coordinates of Minkowskian space-time and $\pm U$ corresponding to proper orthochronous Lorentz transformations.  
