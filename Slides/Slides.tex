...
%\documentclass[10pt,a4paper,notes=hide]{beamer}
...
\item<5->{--Think of the standard Lorentz transformation, always two null directions at $x \pm t$}
\end{itemize}
}
\end{frame}
...
\item<3->{to remove the coordinate singularity in the Schwarzschild solution}
\item<4->{These transformations put the line element in a form where we can take the limit as the energy goes to 0}
\item<5->{It is easily shown with further coordinate transforms that this is Kasner, but it wont be done here}
\end{itemize}
}
\end{frame}
...
\item<3->{This is best shown by calculating the geodesic equations after the Eddington-Finkelstein coordinate transforms, all zero if $u$ is proper time along the geodesic}
\end{itemize}
}
\end{frame}
...
\item<4->{So this is what the seemingly trivial transformation looks like in Cartesian}
...
\note{
\begin{itemize}
\item<2->{Complex Hermitian matrices have 4 independent components, so the element of such a matrix can be used to represent points in Minkowskian space-time.}
...
\note{
\begin{itemize}
\item<2->{This is because the determinant of U is 1}
\item<3->{I want to show you an example calculation of U, to do this we write in component form}
\end{itemize}
}
\end{frame}
...
\item<3->{because the sign doesn't matter is still a solution Eqn(27)}
...
\note{
\begin{itemize}
\item{Connect Minkowskian space to the 2-sphere by stereographic projection, so we can use points on a 2 sphere to think about LTs}
\end{itemize}
}
\end{frame}
...
\note{
\begin{itemize}
\item<3->{As we know, stereographic projection doesn't map the point N at the top of the circle, so that's why we map N to infinity and need to consider the extended complex plane}
...
\item<4->{Extra coord is because we take time into account now, now $\zeta$ has two parameters so x is in terms of two parameters, t just defines the direction}
\end{itemize}
}
\end{frame}
...
\note{
\begin{itemize}
\item<3->{Clear that these equations can be expressed in terms of P and Q}
\end{itemize}
}
\end{frame}
...
\note{
\begin{itemize}
\item<2->{This is derived from the form of the infinitesimal U and from the fractional linear transformation formula}
...
\note{
\begin{itemize}
\item<2->{q/m factors suppressed for convenience}
\end{itemize}
}
\end{frame}
